    ...
    <script>
        ...
        class Star {
            constructor() {
                ...
                // 调整星星大小: "小一号"
                // 原 baseRadius: Math.random() * 2 + 1 (1 to 3)
                // 原 targetRadius: baseRadius * (3 + Math.random() * 4) (3 to 21)
                // 新 baseRadius: Math.random() * 1.5 + 0.5 (0.5 to 2)
                // 新 targetRadius: baseRadius * (1.5 + Math.random() * 1.5) (0.5*1.5=0.75 to 2*3=6)
                // 这使得星星的半径范围大致在 0.75 到 6 之间，比之前小很多
            ...
            applyBoids(starsArray) {
                ...
                let alignment = this.align(starsArray); // 对齐和凝聚效果可以减弱或移除，以突出碰撞
            ...
            // Boids - 分隔 (gentle separation, collision handles hard separation)
            separate(others) {
                let desiredSeparation = this.radius * 2.0; // 期望的最小间距调整
                let steer = new Vector(0, 0);
                let count = 0;
                for (let other of others) {
                    if (other === this) continue;
                    let d = distance(this.pos.x, this.pos.y, other.pos.x, other.pos.y);
                    if (d > 0 && d < desiredSeparation && d < this.radius + other.radius + 5) { // 只在非常近的时候才施加
                        let diff = this.pos.clone().subtract(other.pos);
                        diff.normalize();
                        diff.divide(d * 0.5); // 距离越近，作用力越大
                        steer.add(diff);
                        count++;
                    }
                }
                ...
                if (steer.magnitude() > 0) {
                    ...
                    steer.limit(this.maxForce * 0.5); // 减小Boids分隔力，碰撞为主
                }
                return steer;
            }

            align(others) { /* ... (保持或简化) ... */ return new Vector(); } // 暂时禁用或大幅减弱，突出碰撞
            cohesion(others) { /* ... (保持或简化) ... */ return new Vector(); } // 暂时禁用或大幅减弱

            wander() {
                let wanderRadius = 8; // 减小漫游半径
        ...
        function resolveCollision(s1, s2) {
            ...
            if (xVelDiff * xDist + yVelDiff * yDist >= 0) {
                ...
                if (overlap > 0) {
                    ...
                    const correction1 = correctionNormal.clone().multiply(overlap * (s2.mass / (s1.mass + s2.mass))); // 按质量反比分配修正
        ...
        function animate() {
            ...
            // 绘制连接线 (大幅减少)
            if (connectionDistance > 0) { // 只有当 connectionDistance 有意义时才绘制
                for (let i = 0; i < stars.length; i++) {
                    for (let j = i + 1; j < stars.length; j++) { // 只连接部分星星以减少数量
                        if (Math.random() < 0.02) { // 大约2%的几率尝试连接
                            ...
                            if (d < connectionDistance) {
                                ...
                                ctx.strokeStyle = `rgba(255, 255, 255, ${0.5 - (d / connectionDistance) * 0.4})`; // 更细更透明
                                ctx.lineWidth = 0.3; // 线条更细
                                ctx.stroke();
                            }
                        }
                    }
